...
# model_path = "D:\DigitalHuman\MeloTTS-main\weights\bert_lml_model.onnx"  # 请替换为你的模型的实际路径
# tokenizer_path = r"D:\DigitalHuman\MeloTTS-main\weights\tokenizer.json"
# tokenizer = Tokenizer.from_file(tokenizer_path)
# onnx_providers = ['CUDAExecutionProvider']  # 或者 'CUDAExecutionProvider' 如果你在 GPU 上运行
# session_opts = ort.SessionOptions()
# onnx_params = None  # 任何额外的 ONNX 参数
# # 创建 ONNX 推理会话
# bert_session = ort.InferenceSession(os.path.join(model_path, "bert_lml_model.onnx"),
...
#    # 准备模型输入：转换为 NumPy 数组，并添加批次维度
#    model_inputs = {
#        'input_ids': np.array([inputs.ids], dtype=np.int64),
#        'attention_mask': np.array([inputs.attention_mask], dtype=np.int64)
#    }
#    # 注意：如果你的模型需要 'token_type_ids'，你需要生成或提供它们。
#    # 如果不需要，则可以省略。
#    # 运行 ONNX 模型推理
#    res = bert_session.run(None, model_inputs)[0][0]
#
#    phone_level_feature = []
